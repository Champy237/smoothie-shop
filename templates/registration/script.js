...
class ShoppingCart {
    ...
    checkPasswordMatch() {
        ...
        if (confirmPassword.length > 0) {
            if (password === confirmPassword) {
                confirmInput.style.borderColor = var(--primary-green);
            } else {
    ...
    handleRegister(form) {
        ...
        if (!terms || !privacy) {
            isValid = false;
            this.showNotification('Vous devez accepter les conditions d\'utilisation et la politique de confidentialité.', 'error');
            return;
        }
        ...
        // Log the JSON data that would be sent to Django
        console.log('User registration data for Django:', JSON.stringify(userData, null, 2));

        // Simulate successful registration
        this.showNotification('Inscription réussie ! Redirection vers la connexion...', 'success');
        
        // Store temporary registration success flag
